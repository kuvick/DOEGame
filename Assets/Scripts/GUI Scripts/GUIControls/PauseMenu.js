...
public class PauseMenu extends GUIControl
{
	...
	// Pause Menu Scaling
	private var groupHeightPercent:float = 0.3;			// Height of the entire menu as a percentage of screen height
	private var menuButtonHeightPercent:float = 0.16;			// Height of each button as a percentage of screen height
	private var menuButtonRatio:float = 4;						// Width to height ratio of the button texture (for accurate sclaing)
	private var menuButtonOffsetPercent:float = 0.8;			// The "height" of the button used for positioning to account for whitespace in the textures
	private var resumeButtonHeightPercent:float = 0.2;		// Width of the resume button as a percent of height
	
	private var menuButtonTextOffsetXPercent:float = -0.18;	// Offset X of the button text so it's aligned according to the mockup
	private var menuButtonTextOffsetYPercent:float = 0.43;		// Offset Y of the button text so it's aligned according to the mockup
	
	private var titleOffsetXPercent:float = 0.11;			// Offset X of the meny title so it's aligned according to the mockup
	private var titleOffsetYPercent:float = -0.5;			// Offset Y of the meny title so it's aligned according to the mockup
	...
	public function Initialize()
	{
		...
		levelSelectButton = Rect(verticalBarWidth + (screenWidth - menuButtonWidth)/2, horizontalBarHeight + screenHeight * groupHeightPercent, menuButtonWidth, menuButtonHeight); 
